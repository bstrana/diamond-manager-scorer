      ...
      fetch(`/api/gamestate${cacheBuster}`, {
        ...
        .then(data => {
          if (data && !data.error) {
            ...
            console.log('[FieldPlayers] Initial game state loaded from API');
          }
        })
        .catch((err) => {
          console.debug('[FieldPlayers] Initial API fetch failed, using localStorage:', err.message);
    ...
    const pollInterval = setInterval(() => {
      ...
      fetch(`/api/gamestate${cacheBuster}`, {
        ...
        .then(data => {
          if (data && !data.error) {
            ...
            setGameState(prevState => {
              ...
              if (prevStr !== newStr) {
                console.log('[FieldPlayers] Game state updated from API');
                return normalized;
              }
              return prevState;
            });
            ...
            try {
              ...
              if (savedState) {
                ...
                setGameState(prevState => {
                  ...
                  const newStr = JSON.stringify(normalized);
                  if (prevStr !== newStr) {
                    console.log('[FieldPlayers] Game state updated from localStorage');
                    return normalized;
                  }
                  return prevState;
                });
        ...
        .catch((err) => {
          ...
          console.debug('[FieldPlayers] API fetch failed, trying localStorage:', err.message);
          try {
            const savedState = localStorage.getItem('gameState');
            if (savedState) {
              const parsed = JSON.parse(savedState);
              setGameState(prevState => {
                ...
                if (prevStr !== newStr) {
                  console.log('[FieldPlayers] Game state updated from localStorage (fallback)');
                  return parsed;
                }
                return prevState;
              });
